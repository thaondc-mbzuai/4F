...
class StableDiffusionControlNetInpaintPipeline(StableDiffusionControlNetInpaintPipeline):
    def load_ctrl_projector(self, adapter_ckpt_path, image_proj_ckpt_path, controlnet_ckpt_path, image_emb_dim=512):
    ...
    def __call__(
        ...
        ctrl_prompt_embeds: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        with torch.inference_mode():
            for i, t in enumerate(timesteps):
                ...
                MSEs.append(L2(noise, noise_pred))
                ...
                if num_channels_unet == 4:
                    ...
                    # My adding
                    # if init_mask.shape[1] > 1 and init_mask.shape[1]!=4:
                    #     init_mask = (init_mask.sum(dim=1, keepdim=True) > 0).to(device, prompt_embeds.dtype)
        ...
        image = image.detach() if image.requires_grad else image
        ...
        if not return_dict:
            return (image, has_nsfw_concept, torch.mean(torch.tensor(MSEs)))

        return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)
    # return nn.L1Loss(reduction = 'sum')(matched_output, source)/torch.sum(matched_output != 0)
